...
Vorgeschlagener Ablageort im Repository: `docs/responsive-templates.md`

**Übergabestatus:** Dieses Dokument führt die Anforderungen aus der Abstimmung zusammen. Seine Erstellung bedeutet noch keine Änderung am Repository, keine technische Umsetzung und keine Veröffentlichung. Den tatsächlichen Umsetzungsstand nach Übernahme im bestehenden Ausbauplan führen.
...
Übernimm die vollständige Spezifikation in `docs/responsive-templates.md`. Falls bereits eine inhaltlich entsprechende maßgebliche Spezifikation existiert, konsolidiere die Anforderungen dort und verwende überall denselben Verweis. Keine widersprüchlichen Parallelfassungen anlegen.

Ergänze in der bestehenden `AGENTS.md` eine kurze dauerhafte Regel samt Verweis auf die maßgebliche Spezifikation. Bestehende Regeln bleiben erhalten; die Datei nicht ersetzen.

Geeigneter Regeltext:
