...
| Lokaler Teilschritt | `node --test tools/sot/contract-invariants.test.mjs tools/sot/contracts.test.mjs` besteht 12/12 Fälle. Zusätzliche reine Prüfungen verlangen Scrum-ID-/Quellzuordnung, belegte SoT-Freigabe und Konflikt-/Impact-Ergebnis sowie eindeutige Worker-Claims mit Review und Integration vor Done. Aufrufende Stellen müssen einen aktuellen autoritativen Zustand liefern und Übergänge atomar durchsetzen; CMS-/Analytics-Laufzeitfreigabe wird ausdrücklich verweigert. |
