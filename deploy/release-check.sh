...
mkdir -p "$backup_dir"
$compose exec -T db pg_dump -U vanventure -d vanventure -Fc > "$backup_dir/database-before.dump"
test -s "$backup_dir/database-before.dump"
sha256sum "$backup_dir/database-before.dump"
$compose exec -T web node --input-type=module -e "const response=await fetch('http://127.0.0.1:8787/healthz');if(!response.ok)process.exit(1)"
