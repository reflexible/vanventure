...
test('HTTP login, CSRF, persistent drafts, conflict and private-file protection',async()=>{
  ...
  const releaseApprovals=JSON.stringify([{scope_ref:'REL-TEST-NORWAY-1',approval_ref:'USER-APPROVED-TEST-1',content_ids:['norwegen-2018'],revisions:[1],gates:{content:true,privacy:true,images:true}}]);
  const child=spawn(process.execPath,['editor/server.mjs'],{env:{...process.env,NODE_ENV:'development',DATABASE_URL:database.url,PORT:String(port),EDITOR_ORIGIN:origin,EDITOR_HOST:'127.0.0.1',EDITOR_SECURE_COOKIE:'false',OPENAI_API_KEY:'',GOOGLE_OAUTH_CLIENT_ID:'test-client',GOOGLE_OAUTH_CLIENT_SECRET:'test-secret',GOOGLE_OAUTH_REDIRECT_URI:origin+'/api/cockpit/youtube/callback',YOUTUBE_CHANNEL_ID:'channel-test',COCKPIT_TOKEN_ENCRYPTION_KEY:'a'.repeat(64),EDITOR_RELEASE_APPROVALS:releaseApprovals},stdio:['ignore','pipe','pipe']});
  try{
    ...
    const publisherLogin=await fetch(origin+'/api/login',{method:'POST',headers:{Origin:origin,'Content-Type':'application/json'},body:JSON.stringify({name:'anna',password:'test password 789'})});assert.equal(publisherLogin.status,200);const publisherSession=await publisherLogin.json(),publisherHeaders={Cookie:publisherLogin.headers.get('set-cookie').split(';')[0],Origin:origin,'Content-Type':'application/json','X-CSRF-Token':publisherSession.csrf};
    ...
    assert.equal((await fetch(url+'/release-scopes?revision='+saved.revision,{headers})).status,403);
    const scopes=await (await fetch(url+'/release-scopes?revision='+saved.revision,{headers:publisherHeaders})).json();assert.deepEqual(scopes.scopes,[{scope_ref:'REL-TEST-NORWAY-1',approval_ref:'USER-APPROVED-TEST-1'}]);
    assert.equal((await fetch(url+'/publish',{method:'POST',headers:publisherHeaders,body:JSON.stringify({revision:saved.revision})})).status,403);
    const publish=await fetch(url+'/publish',{method:'POST',headers:publisherHeaders,body:JSON.stringify({revision:saved.revision,scope_ref:'REL-TEST-NORWAY-1'})});assert.equal(publish.status,200);assert.deepEqual(await publish.json(),{published:true,scope_ref:'REL-TEST-NORWAY-1',approval_ref:'USER-APPROVED-TEST-1'});
