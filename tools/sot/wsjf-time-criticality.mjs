...
const FACTORS = Object.freeze({
  fixed_deadline: 5,
  release: 4,
  planned_publication: 3,
  seasonal_relevance: 4,
  external_dependency: 3,
  blocking_work: 4,
  delay_value_loss: 4,
});
const TEMPORAL = new Set(['fixed_deadline', 'release', 'planned_publication', 'seasonal_relevance']);
...
function insideRoot(root, path) { const rel = relative(root, path); return rel && !rel.startsWith(`..${sep}`) && rel !== '..' && !isAbsolute(rel); }

/** Produce a verifiable, non-authorizing Time-Criticality proposal. */
export async function proposeTimeCriticality(input) {
  const errors = [];
  ...
  if (input?.score !== undefined || input?.manual_score !== undefined) errors.push('Time Criticality is proposed from evidence and cannot be manually supplied.');
  ...
  const seen = new Set(); const verified = [];
  for (const item of input.evidence) {
    ...
    if (TEMPORAL.has(item?.factor) && !timestamp(item?.time_reference)) errors.push(`evidence.time_reference is required for ${item?.factor}.`);
    ...
    seen.add(key);
    try {
      const source = await realpath(resolve(root, item.source_ref));
      if (!insideRoot(root, source)) throw new Error('outside');
      const content = await readFile(source, 'utf8');
      if (!content.includes(item.excerpt)) throw new Error('excerpt');
      verified.push({ factor: item.factor, relevance: item.relevance, time_reference: item.time_reference ?? null,
    ...
    } catch { errors.push(`Evidence source or excerpt cannot be verified: ${item.source_ref}.`); }
  }
  if (errors.length) return { valid: false, errors, result: null };
  const strongest = new Map();
  for (const item of verified) { const prior = strongest.get(item.factor); if (!prior || RELEVANCE[item.relevance] > RELEVANCE[prior.relevance]) strongest.set(item.factor, item); }
  ...
  const proposal = { score: scale(raw), confidence,
    rationale: `Evidence-backed proposal from ${[...strongest.keys()].sort().join(', ')} across ${verified.length} verified project source${verified.length === 1 ? '' : 's'}.` };
  if (confidence === 'Low') proposal.uncertainty = {
    missing_information: 'Evidence covers fewer than two Time-Criticality factors.',
    uncertain_assumption: 'The available project evidence may not represent the current delivery window.',
    unanalyzed_area: 'Deadlines, releases, seasonal relevance and external dependencies need further project evidence.',
  };
  return { valid: true, errors: [], result: { evaluated_at: input.evaluated_at, time_criticality: structuredClone(proposal),
    proposal: structuredClone(proposal), evidence: verified, execution_decision: 'NOT_AUTHORIZED', persistence_decision: 'NOT_AUTHORIZED' } };
}
